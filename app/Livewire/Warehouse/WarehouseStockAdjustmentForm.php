...
class WarehouseStockAdjustmentForm extends Component
{
    public $warehouseId;
    public $productId;
    public $newQuantity;
    ...
    public function render()
    {
        ...
        return view('livewire.warehouse.warehouse-stock-adjustment-form');
    }
    ...
    public function updatedNewQuantity($value)
    {
    ...
    public function stockAdjustment()
    {
        ...
        try {
            ...
            $this->dispatch('stockOperationCompleted', ['type' => 'adjustment', 'warehouseId' => $this->warehouseId]);

        } catch (\Exception $e) {
            session()->flash('error', $e->getMessage());
            Log::error('Stock Adjustment Form Error: ' . $e->getMessage());
        }
    }
    ...
    private function calculateAdjustmentDifference()
    {
        if ($this->newQuantity !== null && $this->newQuantity !== '') {
            $this->adjustmentDifference = $this->newQuantity - $this->currentStockBalance;
        } else {
            $this->adjustmentDifference = 0;
        }
    }
